...
const projects: IProjects[] = [
  ...
  },
  {
    ...
    url: "https://vercel.com/lszpilowski-projects/eataly-restaurant",
    image: "image",
    excerpt:
  ...
  return (
    <div className="flex flex-col gap-10 group">
      {projects.map((project, index) => (
        <Link
          href={project.url}
          className="flex flex-col gap-5 hover:cursor-pointer group-hover:opacity-60 hover:!opacity-100 transition-opacity"
          target="/blank"
          key={index}
        >
          ...
          <div className="flex flex-row gap-3">
            {project.techs.map((tech, index) => (
              <p key={index} className="border rounded-full px-3 bg-secondary">
                {tech}
              </p>
            ))}
          </div>
